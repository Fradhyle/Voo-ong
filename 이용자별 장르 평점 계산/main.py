...
def ratings_aggregator(func_args):
    ...
    if rating == 1:
        for genre in genres_columns:
            temp_df.at[0, genre] += movies_genres_onehot_new[
                movies_genres_onehot_new["movie_id"] == movie_id
            ][genre]
    elif rating == -1:
        for genre in genres_columns:
            temp_df.at[0, genre] -= movies_genres_onehot_new[
...
if __name__ == "__main__":
    try:
        ...
        mysql_cnx = create_engine(
            "mysql+pymysql://root:!panda8902@pc.pdmnu.com:3306/mlens", echo=True
        )
        print(mysql_cnx)
        print()
        ...
        while True:
            futures = []
            func_args = (
                (ratings_rescaled.loc[[index]], movies_genres_onehot_new, index)
