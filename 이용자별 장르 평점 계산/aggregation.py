...
def ratings_aggregator(func_args):
    ...
    for index in df_range:
        ...
        if rating == 1:
            for genre in genres_columns:
                temp_df.at[0, genre] += movies_genres_onehot[
    ...
    with warnings.catch_warnings():
        ...
        mysql_cnx = create_engine(
            "mysql+pymysql://root:!panda8902@pc.pdmnu.com:3306/mlens?charset=utf8mb4",
            encoding="utf-8",
        )
...
if __name__ == "__main__":
    try:
        ...
        ratings_rescaled = pd.read_sql(
            f"SELECT * FROM mlens.ratings_rescaled", hive_cnx
        )
        ...
        count_users = pd.read_sql(
            "SELECT COUNT(DISTINCT user_id) FROM mlens.ratings_rescaled", hive_cnx
        )
        ...
        func_args = (
            (
                ...
                ratings_rescaled[ratings_rescaled["user_id"] == user_id],
            )
