...
def show_settings(settings):
    ...
    try:
        ...
        for key in rdb_settings.keys():
            ...
            print(
                "서버 주소 및 포트:",
                rdb_settings[key]["address"] + ":" + rdb_settings[key]["port"],
            )
...
def crawl_select(settings):
    ...
    while True:
        ...
        if choice == 1:
            ...
            movie_basic_info_df = crawler.thread_executor(
                getattr(crawler, "naver_movie_basic_info")
            )
            to_db.create_table(
                settings=settings, table_name="movie-basic", df=movie_basic_info_df
            )
        elif choice == 2:
            pass
        elif choice == 3:
